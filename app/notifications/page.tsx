      ...
      <header className="sticky top-0 z-10 bg-black/80 backdrop-blur-sm border-b border-gray-800">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            {unreadCount > 0 && (
              <p className="text-sm text-[#00f0ff] mt-1">
                未読: {unreadCount}件
              </p>
            )}
          </div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 text-lg">通知はありません</p>
          </div>
        ) : (
          <div className="space-y-4">
            {notifications.map((notification) => (
              <div
                ...
                className={`bg-gray-900 border rounded-xl p-4 cursor-pointer transition-all duration-200 hover:border-[#00f0ff]/50 ${
                ...
                <div className="flex items-start gap-3">
                  <div className="mt-1">
                    {notification.read_at ? (
                      ...
                      <Circle className="w-5 h-5 text-[#00f0ff]" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      {notification.url && (
                        <ExternalLink className="w-4 h-4 text-[#00f0ff] flex-shrink-0 mt-1" />
                      )}
                    </div>
                    <p className="text-gray-300 mb-2">{notification.message}</p>
                    <div className="flex items-center gap-4 text-xs text-gray-500">
                      <span>{formatDate(notification.sent_at)}</span>
                      {notification.step_hours && (
                        <span className="px-2 py-1 bg-[#00f0ff]/10 text-[#00f0ff] rounded">
                          {notification.step_hours === 1
                            ? '1時間後'
                            : notification.step_hours === 24
                            ? 'Day 1'
                            : notification.step_hours === 168
                            ? '1週間突破'
                            : `${notification.step_hours}時間後`}
                        </span>
                      )}
                    </div>
                  </div>
